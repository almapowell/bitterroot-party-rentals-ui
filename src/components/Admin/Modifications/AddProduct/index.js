...
const AddProduct = ({ type, productToUpdate, setSelectedProduct }) => {
    const [newProduct, setNewProduct] = useState(type === UPDATING ? productToUpdate : defaultState);
    ...
    const successfulNotification = () => {
        notification['success']({
            ...
            description:
            `Congradulations. You've added "${newProduct.title}" to the inventory list.`
        })
    };
    ...
    const handleSave = (e) => {
        ...
        if (newProduct.title !== '' || newProduct.image !== '' || newProduct.price !== 0) {
            axios.post('/api/product/create-new', newProduct).then(() => {
         ...
         <div style={{display: 'flex'}}>
             <div style={{width: '50%'}}>
                <Form
                    ...
                    layout="vertical"
                >
                    <Form.Item initialValue={newProduct.title} name="Title" rules={[{ required: true }]} label="Title">
                    <Input
                    ...
                    <Form.Item initialValue={newProduct.image} name="Image" rules={[{ required: true }]} label="Image">
                    <Input
                    ...
                    <Form.Item initialValue={newProduct.price} name="Price" rules={[{ required: true }]} label="Price">
                    <Input
                    ...
                    {
                        type === UPDATING &&
                        <Button onClick={() => setSelectedProduct({})} danger>Cancel</Button>
                    }
                    <Button onClick={(e) => handleSave(e)} type="submit">Save</Button>
                </Form>
             </div>
            <div>
                <h3>Demo</h3>
